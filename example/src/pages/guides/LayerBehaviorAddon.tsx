...
export const LayerBehaviorAddon = () => (
  <DocArticle title="useLayerBehavior 애드온">
    <p className="lead">
      <InlineCode>useLayerBehavior</InlineCode>는 포커스/키보드/오버레이 동작을 빠르게 구현하도록 돕는 선택 기능입니다.
      코어에 강제되지 않으므로 필요할 때만 조합하세요.
    </p>
    ...
    <Section as="h2" id="usage" title="사용 예시">
      <p>
        훅에 필요 데이터를 명시적으로 전달해야 합니다. 특히 현재 열린 다이얼로그 배열과{' '}
        <InlineCode>zIndex</InlineCode>
        를 함께 줘야 최상단 여부를 판단할 수 있습니다.
      </p>
      <CodeBlock language="tsx" code={usageSnippet} />
      <p className="mt-2 text-sm text-muted-foreground">
        훅은 단순히 document 이벤트를 구독하고 정리(clean-up)합니다. 자체적으로 state를 수정하지 않으므로,
        닫기 동작은 직접 호출해야 합니다.
      </p>
    </Section>
