...
export const UsageExamples = () => (
  <div className="space-y-12">
    <div className="space-y-4">
      ...
      <TypographyP>
        <InlineCode>openDialog</InlineCode> 함수를 사용하여{' '}
        <InlineCode>dialogs.ts</InlineCode>에 정의된 다이얼로그를 여는 다양한
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <TypographyP>
          ...
          다이얼로그를 여는 예제입니다. <InlineCode>openDialog</InlineCode>의 첫
          번째 인자로 다이얼로그의 <InlineCode>type</InlineCode>을, 두 번째
          인자로 <InlineCode>props</InlineCode>를 전달합니다.
        </TypographyP>
        <BasicUsageDemo />
      </CardContent>
    </Card>
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <TypographyP>
          ...
          라이브러리는 <InlineCode>z-index</InlineCode>를 자동으로 관리하여
          다이얼로그가 올바른 순서로 표시되도록 보장합니다.
        </TypographyP>
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <TypographyP>
          <InlineCode>onConfirm</InlineCode>과 같은 콜백 함수에 Promise를
          전달하여 비동기 작업을 처리하는 예제입니다. 예를 들어, 확인 버튼 클릭
          시 서버 API를 호출하고, 응답 결과에 따라 다른 다이얼로그(성공 또는
          실패 알림)를 표시할 수 있습니다.
        </TypographyP>
        <AsyncHandlingDemo />
      </CardContent>
